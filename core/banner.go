...
func printUpdateName() {
	...
	txt := nameClr.Sprintf("               - --  Professional Security Suite  -- -")
...
func printOneliner1() {
	...
	txt := textClr.Sprintf("      Advanced Security Research Edition (") + handleClr.Sprintf("@Abdo_CMU") + textClr.Sprintf(")") + spc + textClr.Sprintf("version ") + versionClr.Sprintf("%s", VERSION)
	fmt.Fprintf(color.Output, "%s", txt)
}
